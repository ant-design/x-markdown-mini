...
```
markdown string → marked.lexer → Token[] → tokensToWechatNodes / tokensToAlipayNodes
                → renderTokensToMiniNodes(tokens, adapter, ctx) → MiniNode[]
```

`renderNodes` resolves the target platform (`resolvePlatform`), looks up a `PlatformRenderer` from `RENDERERS` in `src/platforms/index.ts`, and calls `renderer.renderTokens(tokens, ctx)`. There is **no intermediate IR layer and no platform-agnostic transformer**: the single shared renderer in `src/platforms/shared/miniNodeRenderer.ts` does all the semantic mapping (heading dispatch, list flattening, table rebuild, escape, inline walk) and takes a small `MiniNodePlatformAdapter` for the handful of platform quirks. The output `MiniNode` tree (`{ name, attrs, children, animate }`, hast-shaped — rich-text-nodes-compatible by lineage) is rendered by the in-repo `MiniNodeRenderer` component, which dispatches on `node.name` to native primitives (`<text>` / `<view>` / `<image>` / `<scroll-view>`). We render the tree ourselves rather than feeding it to native `<rich-text>`: `<rich-text>` is an HTML-rich-text renderer that would re-impose a tag/attr whitelist, strip events, and block per-node animation — pointless when the tree is already our own structured data.
...
- `src/platforms/shared/miniNodeRenderer.ts` — the **single** transformer (`renderTokensToMiniNodes`); all semantic mapping lives here once. `src/platforms/wechat/tokensToWechat.ts` / `src/platforms/alipay/tokensToAlipay.ts` are thin: each declares a `MiniNodePlatformAdapter` (`linkAttrs` / `imageSrc` / `olAttrs`, optional `node` / `capabilities` — the ~6 lines of real divergence: wechat's `<a data-href>`, alipay's https-only images and dropped `<ol start>`) and a `tokensToXNodes` wrapper. The exported `PlatformCapabilities` objects also carry informational-only fields (`anchorHrefMode` / `requiresHttpsImage` / `supportsOlStart`) that the renderer no longer reads — the behavior is encoded in the adapter functions, not those flags. `marked` is a real dependency bundled via tsup `noExternal`.
...
**Adding a new platform** = copy `src/platforms/wechat/` to `src/platforms/<new>/`, write its `MiniNodePlatformAdapter` (the ~6 lines of `<a>` / `<ol start>` / `<img src>` divergence) plus the thin `tokensToXNodes` wrapper, then in `src/platforms/index.ts` add the new value to the `Platform` union, append a detector to `DETECTORS`, and register a renderer in `RENDERERS`. Because the semantic mapping is shared in `miniNodeRenderer.ts`, a bug fix there is applied once for every platform — only adapter-level quirks are per-platform.
