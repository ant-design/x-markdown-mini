...
`renderNodes` resolves the target platform (`resolvePlatform`), looks up a `PlatformRenderer` from `RENDERERS` in `src/platforms/index.ts`, and calls `renderer.renderTokens(tokens, ctx)`. There is **no intermediate IR layer, no separate adapter step, no platform-agnostic transformer**. Platform quirks (wechat's `<a data-href>` rewrite, alipay's https-only images, alipay dropping `<ol start>`) are baked directly into the per-platform transformer. The output `MiniNode` tree (`{ name, attrs, children, animate }`, hast-shaped) is consumed by either native `<rich-text>` or by the in-repo `NodesRenderer` component, which dispatches on `node.name` to native primitives (`<text>` / `<view>` / `<image>` / `<scroll-view>`).
...
- `src/components/shared/flattenInline.ts` (operates on `MiniNode`) and `src/components/shared/flattenInlineTokens.ts` (operates on marked `Token`) flatten inline trees because mini-program `<text>` cannot nest custom components. The shipped `NodesRenderer` components require flattened input before `setData`.
