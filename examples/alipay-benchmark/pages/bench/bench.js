...
//
//   - x-markdown-mini    : `onRenderComplete` (emitted after the lib's
//                          onRenderComplete fires)
//   - towxml             : synchronous parse + setData callback (no ready event)
//   - marked + rich-text : marked.parse() + setData callback feeding the native
//                          <rich-text> baseline ("no library" lower bound)
//
...
Page({
  ...
  runAll() {
    ...
    setTimeout(() => this._kickTowxml(sample.content), 200);
    setTimeout(() => this._kickRichText(sample.content), 400);
  },
  ...
  _kickRichText(content) {
    ...
    this._starts.rt = t0;
    this.setData({ rtHtml: html }, () => {
      this._record('rt', Date.now() - this._starts.rt);
    });
  },
