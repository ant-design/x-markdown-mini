...
`"@ant-design/x-markdown-mini": 当前包版本` 消费已发布 npm 包。
这让真机/IDE 调试覆盖普通用户安装路径，而不是本地 `dist/` 链接。
...
正式发布使用 **「`dist/` 内容即包根」**（`npm publish ./dist`），
`es/`、`plugins/`、`shared/`、`index.js`、`miniprogram_dist/` 都在包根，故两端写法完全一致：

| 平台 | 解析方式 | 示例 |
| --- | --- | --- |
| 支付宝 | 直接读 `node_modules` 包根，**不认 `package.json#exports`** | `@ant-design/x-markdown-mini/es/Markdown/index`、`.../index.js`、`.../plugins/Latex/index.js` |
...
- **支付宝**：`mini.project.json` 已设 `compileOptions.transpile: {}`——发布的代码是 ES2018
  （含对象展开 / `class`），IDE 需转译。改完依赖版本后重新在 `examples/alipay` 里跑一次
  `npm install`。
