...
function writeKatexTtfDataModule(baseDest) {
  const srcDir = join(root, 'node_modules', 'katex', 'dist', 'fonts');
  if (!existsSync(srcDir)) return;
  const entries = [];
  for (const f of readdirSync(srcDir).sort()) {
    if (!f.endsWith('.ttf')) continue;
    const base64 = readFileSync(join(srcDir, f)).toString('base64');
    entries.push(`  ${JSON.stringify(f)}: ${JSON.stringify('data:font/ttf;base64,' + base64)}`);
  }
  writeFileSync(
    join(baseDest, 'katex-font-data.js'),
    [
      "'use strict';",
      '',
      '// Generated by scripts/copy-component-assets.mjs. Used by Alipay real-device loadFontFace.',
      'module.exports = {',
      entries.join(',\n'),
      '};',
      '',
...
writeKatexTtfDataModule(distRoot);
writeKatexTtfDataModule(distMpRoot);
console.log('[x-markdown-mini] KaTeX ttf fonts copied and data modules generated');
