...
// 1c. KaTeX runtime fonts — WeChat only.
// Alipay 真机 loadFontFace 只认白名单 https CDN ttf（见 shared/loadKatexFonts.ts 的 KATEX_FONT_CDN），
// 包内本地 ttf 与 base64 data URI 均不生效，故不再向 Alipay 包根（distRoot）发 ttf、也不再生成
// base64 数据模块（katex-font-data.js）——净减包体。WeChat 的 /miniprogram_npm/.../katex-fonts 是
// 构建npm 真实产出的可解析路径，作为「本地优先 + CDN 兜底」的本地那一档，仍然发布。
copyKatexTtfFonts(distMpRoot);
console.log('[x-markdown-mini] KaTeX ttf fonts copied to miniprogram_dist (WeChat local fallback)');
